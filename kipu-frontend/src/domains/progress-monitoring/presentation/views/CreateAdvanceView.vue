...
const specialties = ref([
  { name: t('progress-monitoring.advances.specialties.structures'), value: 'structures' },
  { name: t('progress-monitoring.advances.specialties.installations'), value: 'installations' },
  { name: t('progress-monitoring.advances.specialties.architecture'), value: 'architecture' }
]);

const weatherOptions = [
  { label: 'progress-monitoring.weather.sunny', icon: 'pi pi-sun', value: 'sunny' },
  { label: 'progress-monitoring.weather.cloudy', icon: 'pi pi-cloud', value: 'cloudy' },
  { label: 'progress-monitoring.weather.rainy', icon: 'pi pi-cloud-drizzle', value: 'rainy' },
  { label: 'progress-monitoring.weather.storm', icon: 'pi pi-bolt', value: 'storm' }
];
...
const goBack = () => {
  if (isDirty.value) {
    confirm.require({
      message: t('progress-monitoring.create.discardMessage'),
      header: t('progress-monitoring.create.discardHeader'),
  ...
  <div class="create-advance-container">
    ...
    <header class="form-header">
      <div class="title-group">
        <h1>{{ t('progress-monitoring.create.title') }}</h1>
      </div>
    ...
    <div class="form-grid">
      <section class="form-section main-fields">
        <div class="field">
          <label>{{ t('progress-monitoring.create.fields.project') }}</label>
          <Select v-model="form.project" :options="allProjects" optionLabel="name" :placeholder="t('progress-monitoring.create.placeholders.project')" fluid />
        </div>

        <div class="field-row">
          <div class="field">
            <label>{{ t('progress-monitoring.create.fields.date') }}</label>
            <DatePicker v-model="form.date" showIcon iconDisplay="input" fluid />
          </div>
          <div class="field">
            <label>{{ t('progress-monitoring.create.fields.specialty') }}</label>
            <Select v-model="form.specialty" :options="specialties" optionLabel="name" fluid />
          </div>
        </div>

        <div class="field">
          <label>{{ t('progress-monitoring.create.fields.activity') }}</label>
          <InputText v-model="form.activity" :placeholder="t('progress-monitoring.create.placeholders.activity')" fluid />
        </div>

        <div class="field-row">
          <div class="field">
            <label>{{ t('progress-monitoring.create.fields.location') }}</label>
            <InputText v-model="form.location" :placeholder="t('progress-monitoring.create.placeholders.location')" fluid />
          </div>
          <div class="field">
            <label>{{ t('progress-monitoring.create.fields.progress') }}</label>
            <InputText v-model="form.progress" type="number" suffix="%" fluid />
          </div>
        </div>

        <div class="field">
          <label>{{ t('progress-monitoring.create.fields.description') }}</label>
          <Textarea v-model="form.description" rows="4" fluid autoResize />
        </div>

        <div class="field-row">
          <div class="field">
            <label>{{ t('progress-monitoring.create.fields.responsible') }}</label>
            <InputText v-model="form.responsible" fluid />
          </div>
          <div class="field">
            <label>{{ t('progress-monitoring.create.fields.crewSize') }}</label>
            <InputText v-model="form.crewSize" type="number" fluid />
          </div>
        </div>

        <div class="field">
          <label>{{ t('progress-monitoring.create.fields.weather') }}</label>
      ...
      <aside class="form-section photo-section">
        <label>{{ t('progress-monitoring.create.fields.photos') }}</label>
        <FileUpload name="demo[]" multiple accept="image/*" maxFileSize="1000000" :chooseLabel="t('progress-monitoring.create.photos.add')" mode="advanced">
          <template #empty>
            <div class="upload-placeholder">
              <i class="pi pi-images" />
              <p>{{ t('progress-monitoring.create.photos.empty') }}</p>
            </div>
          </template>
        </FileUpload>
      </aside>
    ...
    <footer class="form-footer">
      ...
      <Button :label="t('progress-monitoring.create.submitBtn')" class="submit-btn" @click="submitForm" />
    </footer>
  </div>
</template>
