...
export class AdvanceAssembler {
    ...
    static toEntity(response) {
        return new DailyProgressEntity({
            ...
            activityName: response.activityName || response.taskName || response.activity,
            details: response.details || response.sector || '',
            specialty: response.specialty,
            status: response.status,
            currentPercentage: response.currentPercentage || response.plannedPercentage || response.progress || 0,
