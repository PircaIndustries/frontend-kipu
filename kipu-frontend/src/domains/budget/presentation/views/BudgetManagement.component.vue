...
const loadDashboardData = async () => {
  ...
  try {
    ...
    const currentProjectItems = allItems.filter(item => String(item.projectId) === String(projectsStore.currentProjectId));
    budgetItems.value = currentProjectItems;

    recentMovements.value = currentProjectItems.filter(item => (item.assignedBudget > 0) || (item.executedAmount > 0));
  } catch (error) {
